...
- **吸附**：绘图点击与草稿移动时按住 **Ctrl** 跳过网格吸附；状态栏在按住 Ctrl 时提示。
- **函数图**：可选自动生成零点与极值点（笛卡尔显式 `y=f(x)`，数值近似）；`functionPlot` 支持 `plotOffset` 平移；导出用 `scope` 的 `shift`。
- **椭圆弧**：圆弧子工具「椭圆弧」— 点圆心后在对话框输入两半轴、主轴旋转与起止角；TikZ `arc[x radius=..., y radius=..., rotate=...]`；画布 polyline 预览。
- **`\\foreach`**：`scopeShift` 整体平移导出与预览；向导增加列表范围填入、正文预设与字段说明。
- **工具栏**：各主工具与子菜单弧选项补充悬浮说明（`title`）。
