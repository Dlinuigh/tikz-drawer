...
- **图形**：直线（两点 / 点斜）、圆弧、矩形、圆、椭圆、多段线（可选闭合）、多边形、扇形（外凸饼楔 / 外凸弓形 / 凹弧 / **对称弧楔**（画法同外凸圆心类，边界弧为互补弧）/ 冰激凌）、正多边形、圆锥曲线（采样）、函数/隐式曲线（采样）、`\\foreach` 代码块、点；封闭图形工具栏子菜单可选**新建时无填充或浅色填充**（默认无填充）；**填色拾取**对闭合区域套用填充；**坐标轴**（`axisLine` 单轴或旧版合并 `axes`）；**交点工具**与在交点处**分割**线/多段线/弧（画布侧常驻按钮）；**多选直线**可**合并为填充区域**（闭合环路，画布侧按钮）。
- **画布**：平移视图（Alt / 中键拖拽）、网格与导出网格可配置；**选择工具**下在空白处拖拽可**框选**（`Shift` 追加）；闭合区域**填充**在画布 SVG 中与编译结果一致预览；坐标轴在画布上的 X/Y 显隐由 **macOS View 菜单** 分别控制（文案随状态切换）。
