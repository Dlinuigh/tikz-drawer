...
- **图形**：直线（两点 / 点斜）、圆弧、矩形、圆、椭圆、多段线（可选闭合）、多边形、扇形、正多边形、圆锥曲线（采样）、函数/隐式曲线（采样）、`\\foreach` 代码块、点；矩形/圆/椭圆/多边形/正多边形/扇形**新建时默认浅色实心填充**；**填色拾取**对闭合区域套用填充；**坐标轴**（`axisLine` 单轴或旧版合并 `axes`）；**交点工具**与在交点处**分割**线/多段线/弧（画布侧常驻按钮）；**多选直线**可**合并为填充区域**（闭合环路，画布侧按钮）。
